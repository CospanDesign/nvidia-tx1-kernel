 ...
 * Copyright (c) 2016, NVIDIA Cospan Design LLC.  All rights reserved.
 *
...
#define IMX219_MAX_COARSE_DIFF		      10
...
//	struct camera_common_eeprom_data eeprom[IMX219_EEPROM_NUM_BLOCKS];
//	u8				eeprom_buf[IMX219_EEPROM_SIZE];
...
/* Describes the available controls */
...
/* High Dynamic Range: Not Implemented */
/*
	{
		.ops = &imx219_ctrl_ops,
		.id = V4L2_CID_HDR_EN,
...
/*
	{
		...
		.min = 0,
		.max = IMX219_EEPROM_STR_SIZE,
		.step = 2,
	},
*/
/* Read One Time Programmabe: Not implemented */
/*
	{
		.ops = &imx219_ctrl_ops,
		...
		.min = 0,
		.max = IMX219_OTP_STR_SIZE,
		.step = 2,
	},
*/
/* Fuse ID: Not Implemented */
/*
	{
		.ops = &imx219_ctrl_ops,
		.id = V4L2_CID_FUSE_ID,
...
/*
static inline void imx219_get_coarse_time_short_regs(imx219_reg *regs,
				u32 coarse_time)
{
	regs->addr = IMX219_COARSE_TIME_SHORT_ADDR_MSB;
	regs->val = (coarse_time >> 8) & 0xff;
	(regs + 1)->addr = IMX219_COARSE_TIME_SHORT_ADDR_LSB;
	(regs + 1)->val = (coarse_time) & 0xff;
}
*/

static inline void imx219_get_gain_reg(imx219_reg *regs,
				u16 gain)
{
	regs->addr = IMX219_GAIN_ADDR_MSB;
	regs->val = (gain >> 8) & 0xff;
	(regs + 1)->addr = IMX219_GAIN_ADDR_LSB;
	(regs + 1)->val = (gain) & 0xff;
}

/*
static inline void imx219_get_gain_short_reg(imx219_reg *regs,
				u16 gain)
{
	regs->addr = IMX219_GAIN_SHORT_ADDR_MSB;
	regs->val = (gain >> 8) & 0xff;
	(regs + 1)->addr = IMX219_GAIN_SHORT_ADDR_LSB;
	(regs + 1)->val = (gain) & 0xff;
}
*/
...
//static int imx219_set_coarse_time_short(struct imx219 *priv, s32 val);
...
static int imx219_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (err)
	  dev_dbg(&client->dev, "%s: Error setting camera mode\n", __func__);
		goto exit;
...
*/
	...
	dev_dbg(&client->dev, "%s: Starting stream\n", __func__);
...
static int imx219_set_group_hold(struct imx219 *priv, s32 val)
{
	...
	if (gh_en == SWITCH_ON) {
		err = imx219_write_reg(priv->s_data,
				       IMX219_GROUP_HOLD_ADDR, 0x1);
		if (err)
			goto fail;
	} else if (gh_en == SWITCH_OFF) {
		err = imx219_write_reg(priv->s_data,
				       IMX219_GROUP_HOLD_ADDR, 0x0);
		if (err)
			goto fail;
	}
...
static int imx219_set_gain(struct imx219 *priv, s32 val)
{
	imx219_reg reg_list[2];
	imx219_reg reg_list_short[2];
	int err;
	u32 gain;
	int i = 0;

	/* translate value */
	gain = (u16)camera_common_to_gain(val, IMX219_GAIN_SHIFT);

	dev_dbg(&priv->i2c_client->dev,
		 "%s: val: %d\n", __func__, gain);

	imx219_get_gain_reg(reg_list, gain);
	//imx219_get_gain_short_reg(reg_list_short, gain);
  ...
  /*
	for (i = 0; i < 2; i++) {
		err = imx219_write_reg(priv->s_data, reg_list_short[i].addr,
...
//XXX: This may not be as straight forward as it looks, 0x160 is not really the frame length... I think
static int imx219_set_frame_length(struct imx219 *priv, s32 val)
{
	imx219_reg reg_list[2];
	int err;
...
static int imx219_set_coarse_time_short(struct imx219 *priv, s32 val)
{
	...
	imx219_get_coarse_time_short_regs(reg_list, coarse_time_short);
...
/*
static int imx219_eeprom_device_release(struct imx219 *priv)
{
	int i;

	for (i = 0; i < IMX219_EEPROM_NUM_BLOCKS; i++) {
...
/*
static int imx219_eeprom_device_init(struct imx219 *priv)
{
	char *dev_name = "eeprom_imx219";
	...
	for (i = 0; i < IMX219_EEPROM_NUM_BLOCKS; i++) {
		...
		priv->eeprom[i].brd.addr = IMX219_EEPROM_ADDRESS + i;
		...
		if (IS_ERR(priv->eeprom[i].regmap)) {
			...
			imx219_eeprom_device_release(priv);
			return err;
		}
	}
...
/*
static int imx219_read_eeprom(struct imx219 *priv,
				struct v4l2_ctrl *ctrl)
{
	...
	for (i = 0; i < IMX219_EEPROM_NUM_BLOCKS; i++) {
		err = regmap_bulk_read(priv->eeprom[i].regmap, 0,
			&priv->eeprom_buf[i * IMX219_EEPROM_BLOCK_SIZE],
			IMX219_EEPROM_BLOCK_SIZE);
		if (err)
			return err;
	}

	for (i = 0; i < IMX219_EEPROM_SIZE; i++)
...
/*
static int imx219_write_eeprom(struct imx219 *priv,
				char *string)
{
	int err;
	int i;
...
/*
static int imx219_read_otp_page(struct imx219 *priv,
				u8 *buf, int page, u16 addr, int size)
{
	u8 status;
	int err;

	err = imx219_write_reg(priv->s_data, IMX219_OTP_PAGE_NUM_ADDR, page);
	if (err)
		return err;
	err = imx219_write_reg(priv->s_data, IMX219_OTP_CTRL_ADDR, 0x01);
	if (err)
		return err;
	err = imx219_read_reg(priv->s_data, IMX219_OTP_STATUS_ADDR, &status);
	if (err)
		return err;
	if (status == IMX219_OTP_STATUS_IN_PROGRESS) {
		dev_err(&priv->i2c_client->dev,
	...
	if (err)
		return err;
	if (status == IMX219_OTP_STATUS_READ_FAIL) {
...
/*
static int imx219_otp_setup(struct imx219 *priv)
{
	int err;
	int i;
	struct v4l2_ctrl *ctrl;
	u8 otp_buf[IMX219_OTP_SIZE];

	err = camera_common_s_power(priv->subdev, true);
	if (err)
		return -ENODEV;

	for (i = 0; i < IMX219_OTP_NUM_PAGES; i++) {
		imx219_read_otp_page(priv,
				   &otp_buf[i * IMX219_OTP_PAGE_SIZE],
				   i,
				   IMX219_OTP_PAGE_START_ADDR,
				   IMX219_OTP_PAGE_SIZE);
	}
...
/*
static int imx219_fuse_id_setup(struct imx219 *priv)
{
	int err;
	int i;
	struct v4l2_ctrl *ctrl;
	u8 fuse_id[IMX219_FUSE_ID_SIZE];
	...
	imx219_read_otp_page(priv,
			   &fuse_id[0],
			   IMX219_FUSE_ID_OTP_PAGE,
			   IMX219_FUSE_ID_OTP_ROW_ADDR,
			   IMX219_FUSE_ID_SIZE);
	...
	for (i = 0; i < IMX219_FUSE_ID_SIZE; i++)
		sprintf(&ctrl->string[i*2], "%02x",
			fuse_id[i]);
	ctrl->cur.string = ctrl->string;
...
/*
	case V4L2_CID_EEPROM_DATA:
		err = imx219_read_eeprom(priv, ctrl);
		if (err)
			return err;
		break;
*/
...
/*
	case V4L2_CID_COARSE_TIME_SHORT:
		err = imx219_set_coarse_time_short(priv, ctrl->val);
		break;
*/
/*
...
/*
	case V4L2_CID_EEPROM_DATA:
		if (!ctrl->string[0])
			break;
		err = imx219_write_eeprom(priv, ctrl->string);
		if (err)
			return err;
		break;
*/
			...
			const struct i2c_device_id *id)
{
	...
	client->dev.of_node = of_find_node_by_name(NULL, "imx219");
	...
	/* eeprom interface */
  /*
	err = imx219_eeprom_device_init(priv);
	if (err)
		dev_err(&client->dev,
...
MODULE_AUTHOR("Dave McCoy <dave.mccoy@cospandesign.com.com>");
